...
pub fn read_genome_path_file(input_file: &str) -> Result<HashMap<String, String>> {
    let path = Path::new(input_file);
    let mut reader = csv::ReaderBuilder::new()
        .has_headers(false)
        .delimiter(b'\t')
        .from_path(path)
        .context(format!("Unable to open file: {}", input_file))?;
...
mod tests {
    ...
    fn test_read_genome_path_file() {
        let genome_path_file = "tests/data/genome_paths.tsv";
        let genome_paths =
        ...
        assert_eq!(
            ...
            Some(&String::from("./tests/data/genome1.fna.gz"))
        );
        assert_eq!(
            genome_paths.get("Genome2"),
            Some(&String::from("./tests/data/genome2.fna.gz"))
        );
    }
}
